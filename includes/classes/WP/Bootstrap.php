...
class Bootstrap {
    ...
    public function set_freemius_license_javascript_var() {
        $isPremium = false;
        $sitemap_index_url = "";
        $core_sitemap_available = false;
        if ( function_exists( '\\wp_sitemaps_get_server' ) ) {
            $core_sitemap_available = true;
            $sitemap_index_url = wp_sitemaps_get_server()->index->get_index_url();
        }
        $settings = $this->container['settings_rest']->get_settings( null );
        echo sprintf(
            '<script type="text/javascript">var wptools_seo_schema_is_premium = %b; var wpt_schema_data_settings = %s; var wpt_schema_dirty = false;var wpt_core_sitemap_available = %b; var wpt_sitemap_index_url = "%s";</script>',
            (bool) $isPremium,
            wp_json_encode( $settings ),
            (bool) $core_sitemap_available,
            esc_url( $sitemap_index_url )
        );
    }

    public function add_shortcodes() {
        add_shortcode( 'wpt_schema_breadcrumbs', [$this->container['breadcrumb_shortcode'], 'render'] );
    }
...
<g transform="matrix(.050579 0 0 .050579 .50941 .23548)">
    ...
    <path class="st3" d="m272.1 107.7c38.8-0.6 76.3 14 104.4 40.8l77.7-77.7c-49.2-46.2-114.5-71.6-182.1-70.8-102.9 0-197 58-243.2 150l90.4 70.1c21.5-64.5 81.8-112.4 152.8-112.4z" fill="#ea4335"/>
        ...
        $wp_admin_bar->add_menu( [
            'id'     => 'wpt-schema-checker',
            'parent' => false,
            'title'  => $svg . ' Check Schema Markup',
            'href'   => 'https://search.google.com/test/rich-results?url=' . rawurlencode( $current_url ),
            'meta'   => [
                'onclick' => '',
                'html'    => '',
                'class'   => '',
                'target'  => '_blank',
                'title'   => 'Check the schema markup validity on the page.',
            ],
        ] );
    }
